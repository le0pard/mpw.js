...
const scrypt = (passphrase, salt, N, r, p, keyLen) => {
  ...
  for (let i = 0; i < p; i++) {
    b = b.then(res => (
      new Promise((resolve) => {
        smix(res.subarray(i * 128 * r), r, N, v, x, y)
        resolve(res)
      })
    ))
  }

  return b.then(res => pbkdf2(passphrase, res, 1, keyLen, 'SHA-256'))
}
